...
def run_trials(mountain_car,trials=1, update=False):
	...
	while trial_cnt < trials:
		...
		if(trial_cnt==15 and trial_rewards[trial_cnt-1]<=-time_limit and trial_rewards[trial_cnt-2]<=-time_limit and trial_rewards[trial_cnt-3]<=-time_limit):
			flag= True
			break
	#print len(td_errors)
	#import ipdb; ipdb.set_trace()
...
if __name__=="__main__":
	...
	for values in gridsearch:
			...
			std_dev = np.std(total_rewards,axis=0)
                	...
                	print rewards
                	plot_error(rewards,[i for i in range(0,len(rewards))],"episodes","rewards","rewards vs episodes","more_trials_final_new_mountain_car_rewards_" + str(mountain_car.order) + "_"+ str(mountain_car.alpha)+ "_" + str(mountain_car.epsilon)+"_" + str(ld) + "_"+str(len(total_rewards))+ ".png",std_dev)
