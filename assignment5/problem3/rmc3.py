...
if __name__=="__main__":
	...
	gridsearch = [[3,0.000000001778,0.0001,0.2],[2,0.001778,0.0001,0.5]]
	#for order in orders:
	...
	for values in gridsearch:
			...
			std_dev = np.std(total_rewards,axis=0)
                	...
                	print rewards
                	plot_error(rewards,[i for i in range(0,len(rewards))],"episodes","rewards","rewards vs episodes","reinforce/no_limit_all_more_trials_final_new_mountain_car_rewards_" + str(mountain_car.order) + "_"+ str(mountain_car.alpha)+ "_" + str(mountain_car.epsilon)+"_" + str(ld) + "_"+str(len(total_rewards))+ ".png",std_dev)
