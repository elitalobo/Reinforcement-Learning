...
def plot_error(error,time_steps,xlabel,ylabel,title,filename,y_error=[]):
	   ...
	   #new_alphas = [np.log10(x) for x in alphas]
		...
		plt.axhline(y=-140,linestyle='--',label="max:-140 ")
...
class MountainCar() :
	def __init__(self,order,alpha,epsilon):
		...
		self.value_weights = np.array([np.zeros((1,np.power(self.order+1,2))),np.zeros((1,np.power(self.order+1,2))),np.zeros((1,np.power(self.order+1,2)))])
		...
		self.value_function = np.array([np.zeros((1,np.power(self.order+1,2)))])
	...
	def evaluate_fourier(self,dist,vel):
		...
		return res	
	def evaluate_value_estimate(self,Reward,probabilities,flag,update=False):
		...
		if update:
			for action in range(0,actions_len):
				if(action==action_index[self.action]):
					self.etraces[action] = gamma*ld*self.etraces[action] + (1-probabilities[action])*np.transpose(evaluated_fourier)
				else:
					self.etraces[action] = gamma*ld*self.etraces[action] + (-1.0*probabilities[action])*np.transpose(evaluated_fourier) 
				self.value_weights[action] = self.value_weights[action] + self.alpha*td*self.etraces[action]
			self.e_value = gamma*ld1*self.e_value + np.transpose(evaluated_fourier)
...
if __name__=="__main__":
	...
	gridsearch = [[2,0.0005,10],[3,0.0005,10]]
	#for order in orders:
	...
	for ld_value in lds:
		for values in gridsearch:
			...
			while trial_cnt < trials:
				...
				mountain_car,trial_reward = run_trials(mountain_car,500,True)
			...
			std_dev = np.std(total_rewards,axis=0)
                	...
                	print rewards
                	plot_error(rewards,[i for i in range(0,len(rewards))],"episodes","rewards","rewards vs episodes","sarsa/mountaincar/act_mountain_car_rewards_" + str(mountain_car.order) + "_"+ str(mountain_car.alpha)+ "_" + str(mountain_car.epsilon)+"_"+str(ld)+"_"+str(ld1)+".png",std_dev)
