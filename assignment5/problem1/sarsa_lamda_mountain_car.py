	...
	for ld_value in lds:
		for values in gridsearch:
			...
			std_dev = np.std(total_rewards,axis=0)
                	...
                	print rewards
                	plot_error(rewards,[i for i in range(0,len(rewards))],"episodes","rewards","rewards vs episodes","sarsa_lamda_mountain_car_rewards_" + str(mountain_car.order) + "_"+ str(mountain_car.alpha)+ "_" + str(mountain_car.epsilon)+"_"+str(ld)+".png",std_dev)
