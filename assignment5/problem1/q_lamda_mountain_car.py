...
class MountainCar() :
	...
	def evaluate_value_estimate(self,Reward,flag,update=False):
		...
		target = Reward
		if flag==False:
			target = Reward + gamma*(np.max(action_values))
		...
		if update:
			for action in range(0,actions_len):
                                if(action==action_index[self.action]):
                                        self.etraces[action] = gamma*ld*self.etraces[action] + np.transpose(evaluated_fourier)
                                else:
                                        self.etraces[action] = gamma*ld*self.etraces[action]
	...
	def run_episode(self,td_errors,reset=True,update=False,debug=False):
		...
		self.episodes+=1
		if(update and self.episodes > 250):
			self.epsilon*=0.995
		print "episode ended"
...
if __name__=="__main__":
	...
	lds = [0.1,0.5]
	gridsearch = [[2,0.00316,0.0007],[2,0.000316,0.006]]
	#for order in orders:
	...
	for values in gridsearch:
		for ld_value in lds:
			...
			std_dev = np.std(total_rewards,axis=0)
                	...
                	print rewards
                	plot_error(rewards,[i for i in range(0,len(rewards))],"episodes","rewards","rewards vs episodes","new_qlearn_lamda_mountain_car_rewards_" + str(mountain_car.order) + "_"+ str(mountain_car.alpha)+ "_" + str(mountain_car.epsilon)+"_"+str(ld)+"_" + ".png",std_dev)
