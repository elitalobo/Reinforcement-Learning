...
class CartPole() :
	...
	def apply_poly(self,arr,state):
                lent = len(arr)
                res=[]
                res = 1.0
                for i in range(0,lent):
                        res=res*np.power(state[i],arr[i])
                        i=i+1
                return res
        ...
        def evaluate_fourier(self,dist,ang,vel,ang_vel):
                ...
                #res = np.apply_along_axis(self.apply_poly, 1,arr=self.fourier_vector,state=req)
		res = np.power(req[np.newaxis, :],self.fourier_vector)
                res = res.prod(axis=1)
...
def run_trials(cartpole,trials=1, update=False):
	...
	while trial_cnt < trials:
		...
		print trial_cnt
		print reward
		print cartpole.epsilon
		print cartpole.alpha
		print "Norm:" +  str(np.sum(cartpole.value_weights[0]*cartpole.value_weights[0]))
		print "Norm1:" + str(np.sum(cartpole.value_weights[1]*cartpole.value_weights[1]))
	#print len(td_errors)
	#import ipdb; ipdb.set_trace()
	...
	if(update==False):
		...
		#plot_error(trial_rewards,[i for i in range(0,len(trial_rewards))],"episodes","rewards","rewards vs episodes","poly/poly_cartpole_rewards_" + str(cartpole.order) + "_"+ str(cartpole.alpha)+".png")	
...
if __name__=="__main__":
	...
	orders = [8,9]
	for order in orders:
		alphas = [np.power(10.0,-1.0*x) for x in np.arange(2.0,4.0,1.0)]
		td_errors = []
		for alpha in alphas:
			total_rewards=[]
			trial_cnt=0
			while trial_cnt < trials:
				...
				cartpole,reward = run_trials(cartpole,1000,True)
				trial_cnt+=1
				total_rewards.append(reward)
			trial_rewards = np.mean(total_rewards,axis=0)
			std_dev = np.std(total_rewards,axis=0)
			plot_error(trial_rewards,[i for i in range(0,len(trial_rewards))],"episodes","rewards","rewards vs episodes","qlearn_poly_cartpole_rewards_" + str(cartpole.order) + "_"+ str(cartpole.alpha)+".png",std_dev)
