...
class CartPole() :
    ...
    def evaluate_value_estimate(self,Reward,new_action,flag,update=False):
                ...
                target = Reward
                if flag==False:
                        target = Reward + gamma*(np.matmul(self.value_weights[action_index[new_action]],self.evaluate_tileweight(self.dist,self.ang,self.vel,self.ang_vel)))
    ...
    def run_episode(self,td_errors,reset=True,update=False,debug=False):
        ...
        print(time_steps)
        self.episodes+=1
        if(update and self.episodes>300):
            self.epsilon*=0.99
            self.alpha*=0.95
...
if __name__=="__main__":
    ...
    for values in  gridsearch:
        ...
        print(trial_rewards)
        plot_error(trial_rewards,[i for i in range(0,len(trial_rewards))],"episodes","rewards","rewards vs episodes","longer_sarsa_cartpole_rewards_" + str(cartpole.order) + "_"+ str(cartpole.alpha)+ "_" + str(cartpole.epsilon)+".png",std_dev)    
