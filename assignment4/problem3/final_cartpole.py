...
def cross_entropy(N,K,E,trials=30,n_episodes=30):
        mean_vector, covariance = initialize_mean_cov(N,0)
        trial_cnt=0
        ...
        while trial_cnt< trials:
                ...
                policy_return = []
                try:
                        samples = np.random.multivariate_normal(mean_vector,covariance,size=K)
                except:
		...
		biases=None	
			...
			cartpole = CartPole(0,0,0,0);
                        #cartpole.construct_perceptron(sample)
                        weights1,weights2,biases=cartpole.construct_regression(sample)
			
                        #sample = np.random.multivariate_normal(mean_vector,covariance,size=1)
			...
			total_returns = 0.0
				...
				final_results.append(Return)	
                        avg_return=total_returns/n_episodes
                        #pool.close()
                        #print("Total: " + str(total))
                        policy_return.append((sample,avg_return))
			population_cnt+=1
			#print("Avg Returns: " + str(avg_return))
                policy_ret = policy_return.sort(key=operator.itemgetter(1),reverse=True)
                #print policy_return
                iter_rewards = [x[1] for x in policy_return]

                selected_population = [(np.array(policy_return[cnt][0])) for cnt in range(0,E)]
                mean_vector,covariance = get_mean_covariance(selected_population,trial_cnt+1)
                #avg_mean+= mean_vector
                #avg_covariance += covariance
                print("**************** Trial: " + str(trial_cnt) + " is complete**************** " + str(np.mean(np.array(iter_rewards))))
                trial_cnt+=1
        print initial_mean
        print initial_cov
        return final_results,weights1,weights2,biases

import matplotlib.pyplot as plt
import datetime

if __name__=="__main__":
        new_file = open("cartpole_ce.txt",'w+')
        start = datetime.datetime.now()
        trials=int(sys.argv[1]) or 5  #500
        trial_cnt=0
        while trial_cnt < trials:
                final_results,weights1,weights2,biases = cross_entropy(4,20,10)
		#final_cartpole = CartPole(0,0,0,0)
		#final_cartpole.initialise_perceptron(weights1,weights2,biases)
		#final_results,weights1,weights2,biases = final_cartpole.run_episode(True,True,True)
		#print("policy reward: " + str(policy_reward))
			...
			cnt+=1
                ...
                #ymax = np.max(y)
                #xmin =0
                #xmax = len(final_results)
                #axes.set_xlim([xmin-10,xmax])
                #axes.set_ylim([ymin,ymax])
                #plt.scatter(np.array(x),np.array(y))
                #plt.show()
                #plt.savefig("images/cart_pole_"+ datetime.datetime.now().strftime("%Y-%m-%d-%H-%M")+".png")
                #plt.clf()
                #import ipdb; ipdb.set_trace()
		trial_cnt+=1
        new_file.close()
