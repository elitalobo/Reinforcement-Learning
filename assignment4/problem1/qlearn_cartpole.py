...
class CartPole() :
	def __init__(self,dist,vel,ang_vel,ang,order,alpha,epsilon):
		self.episodes = 0				
		self.alpha = alpha
		self.result=0.0
		self.dist=dist;
	...
	def evaluate_value_estimate(self,Reward,flag,update=False):
		...
		force_values = [np.matmul(self.value_weights[0],self.evaluate_fourier(self.dist,self.ang,self.vel,self.ang_vel)),np.matmul(self.value_weights[1],self.evaluate_fourier(self.dist,self.ang,self.vel,self.ang_vel))]		
		target = Reward
		if flag==False:
	...
	def get_action(self):
		import ipdb;
                ...
                force, proba = random_pick(self.forces,force_probabilities)
		return force
	...
	def run_episode(self,td_errors,reset=True,update=False,debug=False):
		...
		print time_steps
		self.episodes+=1
		if(update) and self.episodes>=25:
			self.epsilon*=0.995
		print "episode ended"
		if(REWARD > 1010):
			import ipdb; ipdb.set_trace()
...
if __name__=="__main__":
	...
	total_rewards=[]
	trial_cnt=0
	gridsearch = [[5,0.001],[4,0.001]]
	for values in  gridsearch:
		total_rewards=[]
		trial_cnt=0
		while trial_cnt < trials:
			cartpole = CartPole(0.0,0.0,0.0,0.0,values[0],values[1],0.05)
			cartpole,trial_reward = run_trials(cartpole,200,True)
			total_rewards.append(trial_reward)
			trial_cnt+=1
		trial_rewards = np.mean(total_rewards,axis=0)
		std_dev = np.std(total_rewards,axis=0) 
		print trial_rewards
		plot_error(trial_rewards,[i for i in range(0,len(trial_rewards))],"episodes","rewards","rewards vs episodes","e_greedy_q_learn_cartpole_rewards_" + str(values[0]) + "_"+ str(values[1])+".png",std_dev)
